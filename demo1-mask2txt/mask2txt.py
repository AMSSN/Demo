...
def mask_to_yolo(mask_dir, save_dir, num_label, num_points, validation=False):
    ...
    for img in imgs:
        ...
        print("{}--{}".format(img, np.array(img_current).max()))
        ...
        for label_id in range(1, num_label + 1, 1):
            ...
            # 2.there will cover the img_temp, and cover the img_current, in next loop, the data will be changed
            ...
            if len(contours) != 0:  # 获取到该类别轮廓
                print("len(contours)={}".format(len(contours[0])))
...
def split_data(input_dir, output_dir, train_ratio, val_ratio, test_ratio):
    pass
...
if __name__ == '__main__':
    ...
    mask_to_yolo(mask_dir, save_dir, class_num, max_points, False)
